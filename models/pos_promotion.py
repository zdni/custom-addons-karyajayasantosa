...
class pos_promotion(models.Model):
    ...
    type = fields.Selection([
        ...
        ('12_discount_product', '12. Discount Product'),
    ...
    discount_multiple_ids = fields.One2many('pos.promotion.discount.multiple', 'promotion_id', string='Discount Multiple')
...
class pos_promotion_discount_multiple(models.Model):
    _name = "pos.promotion.discount.multiple"
    # _order = "product_id"
    _description = "Multiple discount each product"

    product_ids = fields.Many2many('product.product', string='Product', domain=[('available_in_pos', '=', True)], required=1)
    # product_id = fields.Many2one('product.product', 'Product', domain=[('available_in_pos', '=', True)], required=1)
    quantity = fields.Float('Minimum quantity', required=1)
