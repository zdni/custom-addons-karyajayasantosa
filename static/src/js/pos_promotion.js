"use strict";
odoo.define('ti_pos_retail.promotion', function (require) {
    ...
    models.load_models([
        {
            ...
            loaded: function (self, multiple_discount) {
                self.multiple_discount_by_product_id = {}
                var i = 0;
                while( i < multiple_discount.length ) {
                    const product_ids = multiple_discount[i].product_ids;
                    product_ids.forEach(product_id => {
                        if (!self.multiple_discount_by_product_id[product_id]) {
                            self.multiple_discount_by_product_id[product_id] = [multiple_discount[i]]
                        } else {
                            self.multiple_discount_by_product_id[product_id].push(multiple_discount[i])
                        }
                    });
                    i++;
                }
    ...
    models.Orderline = models.Orderline.extend({
        init_from_JSON: function (json) {
            ...
            if (json.product_get_promo_id) {
                this.product_get_promo_id = json.product_get_promo_id;
            }
            ...
            if (json.multiple_discount) {
                this.multiple_discount = json.multiple_discount;
            }
            if (json.discounted_product_id) {
                this.discounted_product_id = json.discounted_product_id;
            }
            if (json.promotion_id) {
                this.promotion_id = json.promotion_id;
            }
            if (json.buyer_promotion) {
                this.buyer_promotion = json.buyer_promotion;
            }
            if (json.frequent_buyer_id) {
                this.frequent_buyer_id = json.frequent_buyer_id;
            }
            if (json.category_get_promo_id) {
                this.category_get_promo_id = json.category_get_promo_id;
            }
            if (json.promotion_special_category) {
                this.promotion_special_category = json.promotion_special_category;
            }
            if (json.promotion_discount_lowest_price) {
                this.promotion_discount_lowest_price = json.promotion_discount_lowest_price;
            }
            if (json.product_id_get_promo) {
                this.product_id_get_promo = json.product_id_get_promo;
            }
            if (json.promotion_multi_buy) {
                this.promotion_multi_buy = json.promotion_multi_buy;
            }
            return res;
        },
        export_as_JSON: function () {
            ...
            if (this.product_get_promo_id) {
                json.product_get_promo_id = this.product_get_promo_id;
            }
            ...
            if (this.multiple_discount) {
                json.multiple_discount = this.multiple_discount;
            }
            if (this.discounted_product_id) {
                json.discounted_product_id = this.discounted_product_id;
            }
            if (this.promotion_id) {
                json.promotion_id = this.promotion_id;
            }
            if (this.buyer_promotion) {
                json.buyer_promotion = this.buyer_promotion;
            }
            if (this.frequent_buyer_id) {
                json.frequent_buyer_id = this.frequent_buyer_id;
            }
            if (this.category_get_promo_id) {
                json.category_get_promo_id = this.category_get_promo_id;
            }
            if (this.promotion_special_category) {
                json.promotion_special_category = this.promotion_special_category;
            }
            if (this.promotion_discount_lowest_price) {
                json.promotion_discount_lowest_price = this.promotion_discount_lowest_price;
            }
            if (this.product_id_get_promo) {
                json.product_id_get_promo = this.product_id_get_promo;
            }
            if (this.promotion_multi_buy) {
                json.promotion_multi_buy = this.promotion_multi_buy;
            }
        ...
        export_for_printing: function () {
            ...
            receipt_line['product_get_promo_id'] = null;
            if (this.promotion) {
                receipt_line.promotion = this.promotion;
                receipt_line.promotion_reason = this.promotion_reason;
                receipt_line.product_get_promo_id = this.product_get_promo_id;
                receipt_line.category_get_promo_id = this.category_get_promo_id;
            }
            return receipt_line;
        },
    ...
    models.Order = models.Order.extend({
        ...
        manual_compute_promotion: function (promotions) {
            ...
            for (var i = 0; i < promotions.length; i++) {
                ...
                if (order.orderlines.length) {
                    ...
                    if (type == '12_discount_product') {
                        order.compute_multiple_discount(promotions[i]);
                    }
                }
            }
        ...
        remove_all_promotion_line: function () {
            ...
            for (var i = 0; i < lines.length; i++) {
                var line = lines[i];
                if (line['promotion'] || line['promotion_discount_total_order'] || line['promotion_discount_category'] || line['promotion_discount_by_quantity'] || line['promotion_discount'] || line['promotion_gift'] || line['promotion_price_by_quantity'] || line['multiple_discount'] || line['promotion_multi_buy'] || line['promotion_discount_lowest_price']) {
                    this.remove_orderline(line);
                }
            }
            for (var i = 0; i < lines.length; i++) {
        ...
        checking_can_discount_by_categories: function (promotion) {
            ...
            for (var i in this.pos.promotion_by_category_id) {
                ...
                if( promotion.id == promotion_line.promotion_id[0] ) {
            ...
            for (var product_id in product_quantity_by_product_id) {
                ...
                if (rules_by_product_id) {
                    for (var i = 0; i < rules_by_product_id.length; i++) {
                        ...
                        if (rule && product_quantity_by_product_id[product_id] >= rule.quantity && promotion.id == rule.promotion_id[0]) {
                            can_apply = true;
                        }
                    }
                }
        ...
        checking_multi_by: function (promotion) {
            ...
            for (var i = 0; i < this.orderlines.models.length; i++) {
                ...
                if (rule && rule['next_number'] <= total_qty_by_product[product_id] && rule.promotion_id[0] == promotion.id) {
                    can_apply = true;
                    break;
                }
        ...
        // 12. multiple discount
        checking_apply_multiple_discount: function(promotion) {
            var can_apply = false;
            var rules = this.pos.multiple_discount_by_product_id;
            var product_quantity_by_product_id = this.product_quantity_by_product_id();
            for (var product_id in product_quantity_by_product_id) {
                var rules_by_product_id = rules[product_id];
                if(rules_by_product_id) {
                    for (let index = 0; index < rules_by_product_id.length; index++) {
                        const rule = rules_by_product_id[index];
                        if(rule && (product_quantity_by_product_id[product_id] >= rule.quantity) && (promotion.id == rule.promotion_id[0])) {
                            can_apply = true;
                        }
                    }
            ...
            if (discount_line_tmp && total_order > 0) {
                ...
                if (product && price != 0) {
                    ...
                    for (let index = 0; index < lines.length; index++) {
                        ...
                        if( line.promotion_discount_total_order  ) {
                            if( line.get_unit_price() >= price ) {
                ...
                if( promotion.id == promotion_line.promotion_id[0] ) {
                    ...
                    if (amount_total_by_category != 0) {
                        ...
                        this.add_promotion(product, price, 1, {
                            ...
                            promotion: true,
                            product_get_promo_id: line.product.id,
            ...
            */
            ...
            for (i in quantity_by_product_id) {
                ...
                for (j in promotion_lines) {
                    if (quantity_tmp <= promotion_lines[j].quantity && quantity_by_product_id[i] >= promotion_lines[j].quantity && promotion.id == promotion_lines[j].promotion_id[0]) {
                ...
                for (var i = 0; i < lines.length; i++) {
                    var line = lines[i];
                    if (line.promotion_discount_by_quantity && line.product_id_get_promo == product_id && line.promotion_id == promotion_line.id) {
                        this.remove_orderline(lines[i]);
                    }
                    if (line.promotion) {
                        continue
                ...
                if (amount_total_by_product != 0 && promotion_line) {
                    this.add_promotion(product, -amount_total_by_product / 100 * promotion_line.discount, 1, {
                        product_id_get_promo: product_id,
                ...
                while (i < discount_items.length) {
                    ...
                    if (product && discount != 0) {
                        this.add_promotion(product, -discount / 100 * discount_item.discount, 1, {
                            promotion: true,
                            product_get_promo_id: discount_item.product_id[0],
                ...
                while (i < gifts.length) {
                    ...
                    if (product) {
                        this.add_promotion(product, 0, -(gifts[i].quantity_free * temp), {
                            promotion: true,
                            promotion_gift: true,
                            product_get_promo_id: product['id'],
                        ...
                        if (price_item_tmp) {
                            ...
                            if (price_discount != 0) {
                                this.add_promotion(product, price_discount, -1, {
                                    ...
                                    product_get_promo_id: price_item_tmp.product_id[0],
                        ...
                        if (promotion_type == 'discount') {
                            ...
                            if (discount != 0) {
                                this.add_promotion(product_service, -discount, 1, {
                                    ...
                                    category_get_promo_id: promotion_line['category_id'][0],
        ...
        compute_discount_lowest_price: function (promotion) { // compute discount lowest price
            ...
            if (line_apply && product_discount) {
                ...
                price = price * promotion.discount_lowest_price / 100;
                this.add_promotion(product_discount, price, -1, {
                    promotion: true,
                    promotion_discount_lowest_price: true,
                    product_get_promo_id: line_apply.product.id,
        ...
        compute_multi_buy: function (promotion) { // compute multi by
            ...
            if (rule_applied && product_discount) {
                ...
                for (var product_id in rule_applied) {
                    ...
                    var sub_price;
                    var price_discount;
                    if (total_qty >= rule_applied[product_id]['next_number']) {
        ...
        compute_multiple_discount: function(promotion) {
            /* 
           ...
           console.log( 'promotion: ', promotion )
            ...
            if(this.orderlines.models.length == 0) return;
            var quantity_by_product_id = {}
            var product = this.pos.db.get_product_by_id(promotion.product_id[0]);
            var i = 0;
            var lines = this.orderlines.models;
            ...
            for (const product_id in quantity_by_product_id) {
                if (Object.hasOwnProperty.call(quantity_by_product_id, product_id)) {
                    let pass = false;
                    const quantity = quantity_by_product_id[product_id];
                    var promotion_lines = this.pos.multiple_discount_by_product_id[product_id];
                    if(!promotion_lines) continue;

                    var promotion_line = null;
                    var j = 0;
                    for(j in promotion_lines) {
                        if( promotion.id != promotion_lines[j].promotion_id[0] ) {
                            pass = true;
                            break;
                        }
                        if(quantity >= promotion_lines[j].quantity) {
                            promotion_line = promotion_lines[j];
                        }
                    }
                    if( pass ) continue;
                    ...
                    for (let index = 0; index < lines.length; index++) {
                        const line = lines[index];
                        if(line.multiple_discount && line.discounted_product_id == product_id && line.promotion_id == promotion_line.id) {
                            this.remove_orderline(line);
                        }
                    ...
                    if(amount_total_by_product != 0 && promotion_line && promotion_line.promotion_id[0] == promotion.id ) {
                        ...
                        this.add_promotion(product, total_discount, 1, {
                            product_get_promo_id: product_id,
                            discounted_product_id: product_id,
                            promotion_id: promotion_line.id,
        ...
        add_promotion: function (product, price, quantity, options) {
            ...
            if (options.product_get_promo_id) {
                line.product_get_promo_id = options.product_get_promo_id;
            }
            if (options.category_get_promo_id) {
                line.category_get_promo_id = options.category_get_promo_id;
            }
            ...
            if(options.multiple_discount) {
                line.multiple_discount = options.multiple_discount;
            }
            if(options.product_id_get_promo) {
                line.product_id_get_promo = options.product_id_get_promo;
            }
            if(options.promotion_id) {
                line.promotion_id = options.promotion_id;
            }
            if(options.promotion_multi_buy) {
                line.promotion_multi_buy = options.promotion_multi_buy;
            }
            if(options.discounted_product_id) {
                line.discounted_product_id = options.discounted_product_id;
            }
            line.price_manually_set = true; // no need pricelist change, price of promotion change the same, i blocked
            line.set_quantity(quantity);
            line.set_unit_price(price);
            line.set_saving(price*-1);
        ...
        get_promotions_active: function () {
            ...
            for (var i = 0; i < this.pos.promotions.length; i++) {
                ...
                else if(promotion['type'] == '12_discount_product' && this.checking_apply_multiple_discount(promotion)) {
                    can_apply = true;
                    promotions_active.push(promotion);
                }
            }
    ...
    var button_promotion = screens.ActionButtonWidget.extend({// promotion button
        ...
        button_click: function () {
            ...
            if (!promotion_manual_select) {
                ...
                var promotions_active = promotion_datas['promotions_active'];
                if (promotions_active.length) {
                    return this.gui.show_popup('popup_selection_promotions', {
                        title: 'Promotions',
    ...
    screens.ProductScreenWidget.include({
        handle_keypress: function(e) {
            ...
            if(mode === Mode.NORMAL) {
                if(key === 't') {
                    document.querySelector('#button-promotion').click();
                }
                if(key === 'b') {
                    document.querySelector('#button-remove-promotion').click();
                }
            }
        }
    })
